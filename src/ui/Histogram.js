...
class Histogram {
     ...
     * @returns {!Histogram}
     */
    withTopLeft(newTop, newLeft=0) {
        return new Histogram(newTop, newLeft);
    }
    ...
    histogramArea(painter, num_wires) {
        let margin_top = 10;
        let margin_X = Config.TOOLBOX_MARGIN_X + this.left;
    ...
    drawYAxisTitle(painter, area) {
        let r = area.withX(this.left).withW(Config.TOOLBOX_MARGIN_X / 2);
        let {x, y} = r.center();
        painter.ctx.save();
